...
/**
 * Created by homework on 2018/3/9.
 */
...
//        Log.i("tag", "after ->" + displayMetrics.widthPixels + " === " + displayMetrics.heightPixels);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (scannerW == 0f || scannerH == 0f) {
            viewFinderRect = new RectF(left, top, SystemUtil.dp2px(context, 180), top + SystemUtil.dp2px(context, 180));
        } else {
    ...
    public QRCoverView setCoverViewCorner(int cornerH, int cornerW, boolean isDp) {
        if (isDp) {
            this.cornerLength = SystemUtil.dp2px(context, cornerH);
            this.cornerWidth = SystemUtil.dp2px(context, cornerW);
        } else {
    ...
    public QRCoverView setCoverViewScanner(int width, int height) {
        //配置扫描框的宽高
        this.scannerH = SystemUtil.dp2px(context, height);
        this.scannerW = SystemUtil.dp2px(context, width);
        //重新获取扫描框的左边距和上边距
