package com.guhecloud.rudez.npmarket.widgit.wavesidebar;
...
public class WaveSideBarView extends View {
    ...
    //dh:这里新增了前两个字。用符号是为了让其ASCII值排名最靠前以保证顺序显示
    private final static String[] DEFAULT_INDEX_ITEMS = {"!★","\"班","#","A", "B", "C", "D", "E", "F", "G", "H", "I",
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0, mIndexItemsLength = mIndexItems.length; i < mIndexItemsLength; i++) {
            ...
            //dh:设置选中字体颜色
            if (i==curPosition)
                mPaint.setColor(Color.parseColor("#ff6715"));
            else
                mPaint.setColor(Color.parseColor("#999999"));
            // draw

            //dh:设置侧边栏显示的文字，这里特殊了前两个，去掉前面的符号，后面的按字母顺序来
            String text;
            if (mIndexItems[i].equals("\"班"))
                text="班";
            else if (mIndexItems[i].equals("!★"))
                text="★";
            else
                text = mIndexItems[i];
            canvas.drawText(
//                    mIndexItems[i], //item text to draw
    ...
    int curPosition=0;//选中导航条位置，默认选中第一个
    //dh:设置当前选中位置
    public void setCurPosition(int position){
        this.curPosition=position;
    }
    //dh:设置当前选中位置（根据type配置）
    public void setCurPosition(String type){
        for (int i=0;i<mIndexItems.length;i++){
            if (mIndexItems[i].contains(type)){
                if (i==curPosition){//如果已经是是当前选中位置就不用重复绘制
                    return;
                }
                //设置当前选中位置
                setCurPosition(i);
                invalidate();//必须刷新重新绘制，否则界面没有效果
                return;
            }
        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mStartTouching && !mLazyRespond && onSelectIndexItemListener != null) {
                    onSelectIndexItemListener.onSelectIndexItem(mIndexItems[mCurrentIndex]);
                    setCurPosition(mCurrentIndex);//滑动是改变选中位置
                }
                invalidate();
                return true;
